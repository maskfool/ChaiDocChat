...
export default function ChatArea({ messages, onSend, dark, isLoading }) {
    ...
    <div className="flex-1 min-h-0 flex flex-col">
      <div className={`flex-1 min-h-0 max-h-full overflow-y-auto px-4 py-6 overscroll-contain`}>
        <div className="space-y-1">
          {messages.map(m => (
            <MessageBubble key={m.id} role={m.role} time={m.time} dark={dark}>
              {m.text}
            </MessageBubble>
          ))}
          <div ref={endRef} />
        ...
        <div className="max-w-4xl mx-auto flex items-end gap-2">
          <textarea
            ...
            className={`flex-1 resize-none rounded-xl border px-3 py-2 text-sm focus:outline-none focus:ring-2 ${
              dark
                ? 'border-white/20 bg-black/40 text-neutral-100 focus:ring-white/10'
